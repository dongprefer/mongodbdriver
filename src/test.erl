...
test_insert()->
    ...
    Info = #datacollection{'_id' = Id, 'user_id' = <<"2222222222">>, type = <<"1">>, data = <<"3">>}, 
    datacollection_helper:create(Info).

test_find()->
    Info = #datacollection{'user_id' = <<"2222222222">>, type = <<"1">>, data = <<"3">>},
...
test_delete()->
    %Info = #datacollection{'_id' = <<"-576460644743358709">>, type = <<"1">>, data = <<"3">>},
    Id = <<"-576460644743358709">>,
    datacollection_helper:clear(Id).

test_save_old()->
    Info = #datacollection{'_id' = <<"-576460275395216712">>,'user_id' = <<"2222222222">>, type = <<"1">>, data = <<"nihao">>, ts= <<>>, time_created= <<>>, time_updated
= <<"7">>},
    datacollection_helper:save(Info).

test_save_new()->
    Id =  list_to_binary(integer_to_list(erlang:monotonic_time())),
    Info = #datacollection{'_id' = Id,'user_id' = <<"2222222222">>, type = <<"1">>, data = <<"nihao">>, ts= <<>>, time_created= <<>>, time_updated
= <<"7">>},
    datacollection_helper:save(Info).

test_fetch_limit(Skip, Limit)->
    Info = #datacollection{'user_id' = <<"2222222222">>, type = <<"1">>},
...
test_user_info()->
          Start = 1,
          Limit = 1,
          CountQuery = {'$query', #user_info{}, '$skip', Start, '$limit', Limit},
          user_info_helper:fetch(CountQuery).
